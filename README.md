...
| 自動解法 | Kociemba の 2 フェーズ法で解法手順を生成し、ステップ実行アニメーション表示 |
| 解法・履歴表示 | 解法手順の一覧と操作履歴を表示 |

> **注**: Web 版もブラウザ内で 2 フェーズ法を実行します（CLI の kociemba と同じ考え方の探索を JavaScript で実装。外部ライブラリ・サーバー不要）。ランダム 25 手のスクランブルに対して**平均 21 手・最大 23 手**、1 回の探索は 40 ms 程度です。探索表の生成にページ読み込み後 0.4 秒ほどかかりますが、これは裏で先に済ませています。
>
> LBL（Layer-by-Layer）方式のソルバーも `cube-solver.js` に残してあり、2 フェーズ法が時間内に解を見つけられなかった場合の保険として使われます（こちらは平均 127 手）。
...
│   ├── index.html
│   ├── app.js                         # UI コントローラ
│   ├── cube-solver.js                 # キューブ状態モデル + LBL ソルバー（保険）
│   ├── two-phase.js                   # Kociemba 2 フェーズ法ソルバー（既定・20 手前後）
│   ├── cube-renderer.js               # 2D 展開図 + CSS 3D プレビュー描画
│   └── style.css
├── test_solver.js                     # 両ソルバーの回帰テスト（node test_solver.js [試行回数]）
